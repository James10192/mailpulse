...
import { EVOLUTION_REQUEST_MARGIN_MS, MAX_SEND_TIMEOUT_MS, sendTimeoutMs, typingDelayMs } from "./baileys-typing";
...
test("between the bounds the delay follows the typing speed and its jitter", () => {
  // 30 characters at ~5 chars/s with a ~1.25 jitter: about 7.5 s.
  const fastest = typingDelayMs(30, "bulk", sequence(0.999, 0.999));
  assert.ok(Math.abs(fastest - 7_500) < 10, String(fastest));
...
test("sends looped inside one request type briefly, between 0.5 and 1.5 s", () => {
  assert.equal(typingDelayMs(0, "inline_batch", () => 0), 500);
  assert.equal(typingDelayMs(100_000, "inline_batch", () => 0.999), 1_500);
});
...
test("a send waits for its typing delay plus a fixed margin, under 20 s at most", () => {
  ...
  assert.equal(MAX_SEND_TIMEOUT_MS, sendTimeoutMs(typingDelayMs(1_000_000, "bulk")));
});
