...
function FilonLogo() {
  return (
    <span className="inline-flex items-center gap-2">
      <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#5b4cf6] text-white shadow-sm">
        <span className="text-sm font-bold leading-none">F</span>
      </span>
      <span className="font-semibold text-zinc-900 dark:text-zinc-100">Filon</span>
    </span>
  );
}
...
export function IntegrationsClient({
  ...
  return (
    <div className="page-stack app-shell-safe max-w-4xl">
      <div>
        <h1 className="text-2xl font-semibold text-zinc-900 dark:text-zinc-100">Intégrations</h1>
        <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
          Connectez Filon à MailPulse pour préparer les relances de recouvrement.
        </p>
      </div>
        ...
        <CardHeader className="border-b border-zinc-200 bg-zinc-50/80 dark:border-zinc-800 dark:bg-zinc-900/40">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <CardDescription>
                Filon garde le cockpit commercial. MailPulse prépare l&apos;email, WhatsApp et le suivi communication.
              </CardDescription>
            </div>
            <Badge variant={keys.length > 0 ? "success" : "filon"}>
              <ShieldCheck className="h-3 w-3" />
              {keys.length > 0 ? "Prêt" : "À connecter"}
            </Badge>
          </div>
        </CardHeader>
                ...
                <div key={key.id} className="flex flex-col gap-3 rounded-lg border border-zinc-200 p-4 dark:border-zinc-800 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-zinc-500">
                      ...
                      {key.lastUsedAt ? ` · utilisée le ${new Date(key.lastUsedAt).toLocaleDateString("fr-FR")}` : " · jamais utilisée"}
                    </p>
                  </div>
