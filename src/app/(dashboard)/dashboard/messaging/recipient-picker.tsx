...
import { Check, ChevronsUpDown, Loader2, Tag, UserPlus, Users } from "lucide-react";
...
export function RecipientPicker({
  ...
  function openNewContactDialog() {
    ...
    setDialogOpen(true);
    setContactError(null);
    setNewContact((current) => ({ ...current, phone: phone || "" }));
  }
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="flex flex-wrap gap-2">
          ...
          >
            Campagne WhatsApp ({contactsWithPhone})
          </Button>
        </div>
            ...
            <Popover open={comboboxOpen} onOpenChange={setComboboxOpen}>
              <PopoverTrigger asChild>
                ...
                >
                  <span className="min-w-0 truncate text-left">
                    {selectedContact
                      ? `${getContactName(selectedContact)} · ${selectedContact.phone}`
                      : "Choisir ou créer un contact WhatsApp"}
                  </span>
              ...
              <PopoverContent className="w-[min(36rem,calc(100vw-2rem))] p-0" align="start">
                <Command>
                  ...
                  <CommandList>
                    <CommandGroup>
                      <CommandItem value="nouveau contact whatsapp" onSelect={openNewContactDialog}>
                        <UserPlus className="h-4 w-4 text-orange-500" />
                        <div className="min-w-0 flex-1">
                          <p className="font-medium">Nouveau contact</p>
                          <p className="text-xs text-zinc-500">Créer un destinataire WhatsApp</p>
                        </div>
                      </CommandItem>
                    </CommandGroup>
                    <CommandSeparator />
