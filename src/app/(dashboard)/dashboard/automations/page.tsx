  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { name: "Serie de bienvenue", desc: "Envoyez une sequence d'emails aux nouveaux abonnes", trigger: "Nouvel abonne" },
          { name: "Re-engagement", desc: "Ciblez les contacts inactifs depuis 30 jours", trigger: "Inactivite" },
          { name: "Anniversaire", desc: "Email automatique pour l'anniversaire du contact", trigger: "Date" },
        ].map((automation) => (
          <button
            key={automation.name}
            className="p-5 rounded-xl border border-dashed border-zinc-300 dark:border-zinc-700 hover:border-orange-500/50 text-left transition-colors group bg-white dark:bg-transparent"
          >
            <Zap className="h-5 w-5 text-zinc-400 dark:text-zinc-500 group-hover:text-orange-500 mb-3 transition-colors" />
            <h3 className="font-medium text-sm text-zinc-900 dark:text-zinc-100">{automation.name}</h3>
